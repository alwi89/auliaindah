...
$('#grosirreset').click(function(){
	$.ajax({
		url: "grosir_proses.php",
		data: {'aksi_grosir':'reset'},
		type: 'POST',
		dataType: 'json',
		beforeSend: function(){
			$("#loading_grosir").show();
...
var table_cart_grosir = $('#data_cart_grosir').DataTable({
					"bPaginate": false,
					"aProcessing": true,
					"aServerSide": true,
...
function data_barang_grosir(){
	$.ajax({
			...
			success: function(datas){
					...
					if(datas[0]!=null){
						var hasil = '<select name="grosirkode" id="grosirkode" class="form-control" data-provide="typeahead">';
...
function data_sales(){
	$.ajax({
			...
			success: function(datas){
					if(datas[0]!=null){
						var hasil = '<select name="grosirsales" id="grosirsales" class="form-control" data-provide="typeahead">';
...
function proses_simpan_grosir(){
	$.ajax({
		url: "grosir_proses.php",
		data: {'aksi_grosir':'tambah', 'no_nota_grosir':$('#no_nota_grosir').val(), 'tgl_keluar_grosir':$('#tgl_keluar_grosir').val(), 'sales':$('#grosirsales').val(), 'dibayar':$('#grosirdibayar').val(), 'kekurangan':$('#grosirkekurangan').val(), 'tgl_tempo':$('#grosirtgl_tempo').val(), 'potongan':$('#grosirpotongan').val(), 'ket_potongan':$('#grosirket_potongan').val(), 'total':$('#total_grosir').val()},
		type: 'POST',
		dataType: 'json',
		beforeSend: function(){
                ...
                <table width="80%">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="button" class="btn btn-primary" value="Reset" id="grosirreset" />
                        </td>
                    </tr>      
                </table>
                </form>
