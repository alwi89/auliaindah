...
if(isset($_POST['aksi_rec'])){
	if($_POST['aksi_rec']=='tambah'){
		...
		$a = mysql_query("insert into return_ecer(no_nota, tgl_return, username, no_nota_ecer) values('$no_nota', '$tgl_keluar', '$username', '$no_nota_ecer')");
		if($a){
			...
			foreach ($contents as $id=>$qty) {
				...
				$total += ($harga*$qty);
				$d = mysql_query("insert into return_ecer_detail(kode, qty, no_nota, harga, harga_modal) values('$c[kode]', '$qty', '$no_nota', '$harga', '$c[harga_modal]')");
				if($d){
					//update penambahan saldo barang rusak di cabang yang menerima return
					...
					$hit = mysql_query("select sum(qty) as jml, sum(harga*qty) as total from return_ecer_detail where no_nota='$no_nota'");
					$hHit = mysql_fetch_array($hit);
					mysql_query("update return_ecer set total='$hHit[total]', jumlah='$hHit[jml]' where no_nota='$no_nota'");
					unset($_SESSION['brec']);
