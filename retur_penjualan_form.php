...
$(function(){
	...
	keranjang = $('#data_cart').DataTable({
		...
		"aProcessing": true,
		"aServerSide": true,
		"ajax": "retur_penjualan_proses.php?no=<?php echo $_GET['no']; ?>&cart",
			"columns": [
				{ "data": "nama_barang" },
				...
				{ "render": function(data, type, full){
						...
						$('#total').val(total);
						if(full['nama_member']!=null){
							$('#member').val(full['nama_member']);
						}
						var dibayar = parseInt(full['dibayar']);
						$('#t_dibayar').autoNumeric('set', dibayar);
						$('#dibayar').val(dibayar);
						var kekurangan = total-dibayar;
						if(kekurangan>=0){
							$('#t_kekurangan').autoNumeric('set', kekurangan);
						...
						if(full['tgl_tempo']!=null){
							var tgl_tempo = full['tgl_tempo'].substr(8, 2)+'/'+full['tgl_tempo'].substr(5, 2)+'/'+full['tgl_tempo'].substr(0, 4);
...
function cek_barang(){
	$.ajax({
		url: "retur_penjualan_proses.php?data_barang",
		data: {'kode':$("#kode").val()},
		type: 'POST',
		dataType: 'json',
...
function addToCart(){
	if($('#nama_barang').val()!="" && $('#t_sub').val()!=""){
		$.ajax({
			url: "retur_penjualan_proses.php?add",
			data: {'kode':$("#kode").val(), 'harga':$("#harga").val(), 'jml':$("#jml").val(), 'no_nota':$('#no_nota').val()},
			type: 'POST',
			dataType: 'json',
			success: function(datas){
				if(datas[0]==null){
...
function removeCart(kode){
	$.ajax({
			url: "retur_penjualan_proses.php?del",
			data: {'kode':kode, 'no_nota':$('#no_nota').val()},
			type: 'POST',
			dataType: 'json',
			success: function(datas){
				if(datas[0]==null){
...
function editCart(event, jml, kode){
	if(event.keyCode == 13){
		if(jml!=""){
			$.ajax({
				url: "retur_penjualan_proses.php?edt",
				data: {'kode':kode, 'jml':jml, 'no_nota':$('#no_nota').val()},
				type: 'POST',
				dataType: 'json',
				success: function(datas){
					if(datas[0]==null){
             ...
             <div id="message_barang_keluar"></div>
              <div>
              	<div style="float:left;background:#FFFFFF;width:70%;padding:10px;">
                	<a href="?h=retur_penjualan"><input type="button" class="btn btn-danger" value="&lt; kembali ke halaman sebelumnya" /></a>
                    <br /><br />
                    ...
                    <div style="background:#FFFFFF;margin-top:10px;padding:10px;text-align:right;">
                    	<div class="form-group">
                            <div class="form-inline">
                                No Nota
                                <input type="text" name="no_nota" id="no_nota" value="<?php echo $_GET['no']; ?>" class="form-control" tabindex="5" readonly autocomplete="off" />
                            </div>
                        </div>  
                    	<div class="form-group">
                            <div class="form-inline">
                                Member
                                <input type="text" name="member" id="member" class="form-control" tabindex="5" disabled autocomplete="off" />
                            </div>
                        </div>             
                        <div class="form-group">
                            <div class="form-inline">
                                Tgl Keluar
                                <input type="text" name="tgl_keluar" id="tgl_keluar" required class="form-control"  tabindex="6" autocomplete="off" />
                            </div>
                        </div>       
                    	<div class="form-group">
                            <div class="form-inline">
                        ...
                        <div class="form-group">
                            <div class="form-inline">
                                Dibayar
                                <input type="text" name="t_dibayar" required id="t_dibayar" class="form-control" tabindex="7" autocomplete="off" />
                        ...
                        <div class="form-group">
                            <div class="form-inline">
                                Tgl Tempo
                                <input type="text" name="tgl_tempo" id="tgl_tempo" class="form-control" tabindex="8" autocomplete="off" />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="form-inline">
                        ...
                        <div class="form-group">
                            <div class="form-inline">
                                <input type="submit" value="simpan perubahan" tabindex="9" class="btn btn-primary" />
                            </div>
                        </div>
                    </div>
                </div>
